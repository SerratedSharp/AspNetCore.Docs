...
This article explains how to setup a Blazor client-side component to interact with JavaScript (JS) using the `[JSImport]`/`[JSExport]` interop API released for apps that adopt .NET 7 or later.  See [Interoperate with JavaScript from .NET WebAssembly](xref:client-side/dotnet-js-interop.ms) for additional `[JSImport]`/`[JSExport]` interop details and examples.
